...
| 2024/12/26    | ヘッダーコンポーネントの作成、Vercel へのデプロイ、実測値の人口データのみをグラフに表示する修正       | 3.0 時間 (21:00~25:00)              | 1.0 時間                                |
| 2024/12/27-28 | 体調不良のためおやすみ                                                                                | 0.0 時間                            | 0.0 時間                                |
| 2024/12/29    | テストの作成、人口の割合を表示する機能の追加                                                          | 2.0 時間 (22:00~25:00)              | 1.0 時間                                |
| 2024/12/30    | 全体レイアウトの調整                                                                                  | 1.5 時間 (19:00~19:30, 25:00~26:00) | 0 時間                                  |
| 2024/12/31    | データ取得時に API キーを秘匿化、エラーハンドリングや命名修正等のリファクタリング、ドキュメントの充実 | 3 時間 (14:00~17:00)                | 0 時間                                  |
...
- ヘッダーコンポーネントの作成
- Vercel へのデプロイ
- 実測値の人口データのみをグラフに表示する修正

簡単なヘッダーコンポーネントを作製し、Vercel へのデプロイを行った。
ハッカソンで Vercel にデプロイした際に、リージョンが東京になっておらず遅延がすごいことになった経験を忘れず、リージョンを東京にしておいた。
実装面が簡単だったため、グラフに推測値の人口データが表示されていたところを、実測値の人口データのみを表示するように修正した。それに伴い、Swagger の Schema を意識して、型定義の追加や命名変更を行った。
今まで boundaryYear の変数を使っていなかったため、何に使うのかわかっていなかったが、Swagger の Schema を見直したらちゃんと定義されてあった。ちゃんとドキュメント見ようね。

今度こそ本当にテストだけが残ったので、明日はテストを書き始める。後半で巻き返せたので進捗はいい具合。
時間余れば、地方絞り込み機能を実装したい。

### 10, 11 日目 (2024/12/27-28)

体調不良のためおやすみ。あまりにきついので多分インフルだけど、近くの病院空いてなかったのでわからない。5 キロ先の病院も紹介状必要みたいで悲しい。
...
熱だけ収まったので、休憩多めで作業。テスト作成がよくわかっていなかったので、簡単なヘッダーと、グラフのテストを作成。グラフはモックデータも追加した。
人口の割合を表示する機能については、Swagger のドキュメントを見直していると、rate という使った覚えのない変数があったことに気づいた。総人口では null であることに気づいたので、型定義ファイルを修正し、rate が null でない場合はグラフの点に対してマウスホバー時に割合を表示する機能を追加した。

### 13 日目 (2024/12/30)

- 全体レイアウトの調整

昨日より体調が悪く、進められず。19:00-19:30 で作業。きつかったので仮眠をとり、25:00-26:00 で作業。
テストを除いて、最低限提出できるレイアウトに調整。
調整していて、チェックボックスにチェックを入れる度にほぼ全てのコンポーネントが再レンダリングされていることに気づいた。
おそらく、状態管理ライブラリとかを使った方がよさそう。今からの導入は厳しそう。
明日いちからクローンして、ちゃんと動くかや README へ環境構築や説明を追記する。

### 14 日目 (2024/12/31)

- データ取得時に API キーを秘匿化
- エラーハンドリングや命名修正等のリファクタリング
- ドキュメントの充実

クライアント側で読み込める環境変数をヘッダーに含めているため、ネットワークタブから API キーを確認できてしまう問題を発見。
そのため、API routes を用いて環境変数を読み込んだデータフェッチを行い、それを経由してさらに api/◯◯ からデータフェッチを行うように修正。
これにより、クライアント側から API キーを確認できないようにした。

また、エラーハンドリングや命名修正等のリファクタリングを行った。
...
パッケージを入れ直し、サーバー起動や Biome によるフォーマットチェックコマンドを追加し直した。
`deno add` コマンドでパッケージをインストールしたため、Deno で動いていると思っていたが、deno.json がなく、package.json があったため、package.json 側にパッケージが追加されていたのが原因だった。
...
- API routes を経由したデータフェッチ

クライアント側で読み込める環境変数をヘッダーに含めているため、ネットワークタブから API キーを確認できてしまう問題を発見。
そのため、API routes を用いて環境変数を読み込んだデータフェッチを行い、それを経由してさらに api/◯◯ からデータフェッチを行うように修正。
これにより、クライアント側から API キーを確認できないようにした。

実装までにあまり時間がかけられなかったことと、NextResponse, NextRequest の使い方がわからなかったため、コード生成を利用。
すでに実装済みである fetch◯◯ の関数との連結と、環境変数の扱い、エラーハンドリング以外については、ほぼ出力のままで実装できた。
それらを自力で実装および修正し、完成させた。
